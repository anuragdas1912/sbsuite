...
export default function CommercialPortal() {
  ...
  const loadDatabase = async (tenantId: string) => {
    try {
      ...
      if (currentTenant && currentTenant.role === 'commercial') {
        ...
        const fallback = allTenants.find(t => t.role === 'commercial');
        if (fallback) {
  ...
  useEffect(() => {
    const tenantId = localStorage.getItem('sb_current_tenant_id') || 't3';
  ...
  const t = {
    ...
    hi: {
      ...
      msgTab: 'संदेश',
      welcome: 'स्वागत है,',
      unitLabel: 'दुकान नंबर',
      rentSummary: 'दुकान लीज एवं किराया विवरण',
      rentLabel: 'मूल दुकान किराया',
      ...
      rentAg: 'दुकान लीज एग्रीमेंट (Lease Agreement)',
          ...
          <div className="bg-[#0E0F12] border border-[#1B1C21] rounded-xl p-5 sm:p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div className="p-4 bg-[#060608] border border-[#1B1C21] rounded-xl flex justify-between items-center">
                ...
                <button
                  onClick={() => handleDownloadDoc('lease_agreement.pdf', 'SHOP LEASE AGREEMENT (दुकान लीज)')}
